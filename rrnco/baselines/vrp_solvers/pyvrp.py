...
def instance2data(instance: TensorDict, scaling_factor: int) -> ProblemData:
    ...
    num_depots = instance.get("num_depots", 1)
    ...
    distance_matrix = scale(instance["distance_matrix"], scaling_factor)  # num_locs
    duration_matrix = scale(instance["duration_matrix"], scaling_factor)  # num_locs
    ...
    if instance["open_route"]:
        ...
        distance_matrix[:, :num_depots] = 0
        duration_matrix[:, :num_depots] = 0  # TODO: correct? (i guess so)
    ...
    if instance["backhaul_class"] == 1:  # VRP with backhauls
        ...
        distance_matrix[np.ix_(backhaul, linehaul)] = MAX_VALUE
