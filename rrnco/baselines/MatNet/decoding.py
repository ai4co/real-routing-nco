...
class DecodingStrategy(metaclass=abc.ABCMeta):
    ...
    def step(
        ...
        **kwargs,
        ...
        if first_action:
            selected_action = self.sampling(logprobs, mask)
        else:
    ...
    def sampling(logprobs, mask=None):
        ...
        probs = logprobs.exp()
        assert not torch.isnan(probs).any().item()  # check probs for NaN
        assert not torch.isinf(probs).any().item()  # check probs for inf
        ...
        if mask is not None:
            ...
            assert not torch.isnan(selected).any().item()  # check selected for NaN
            assert not torch.isinf(mask).any().item()  # check mask for inf
            assert not torch.isinf(selected).any().item()  # check selected for inf
...
def process_logits(
    ...
    """
    assert not torch.isnan(logits).any().item(), "logits contains NaN"
    # Tanh clipping from Bello et al. 2016
    if tanh_clipping > 0:
        logits = torch.tanh(logits) * tanh_clipping
    assert not torch.isnan(logits).any().item(), "tanh with logits contains NaN"
    ...
    logits = logits.float()  # Convert to FP32 for Stability
    logits = logits / temperature  # temperature scaling
    logits = logits - logits.max(dim=-1, keepdim=True).values  # for numerical stability
    # logits = logits + torch.randn_like(logits) * 1e-4  # Add Noise to Avoid Uniform Values
    logits = torch.clamp(logits, min=-50.0, max=-1e-4)
    ...
    logits = F.log_softmax(logits, dim=-1)  # Convert to log probabilities
    # logits = F.softmax(logits, dim=-1) + 1e-6  # Avoid division by zero
    # logits = torch.log(logits)
    assert not torch.isnan(logits).any().item(), "Log-logits contains NaN"
