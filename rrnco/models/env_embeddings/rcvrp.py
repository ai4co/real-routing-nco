...
class RVRPInitEmbedding(nn.Module):
    ...
    def __init__(
        ...
        if not self.use_dist:
            ...
            if self.use_coords:
                ...
                self.demand_init = nn.Linear(1, embed_dim, linear_bias)
                self.combine_row_embed = nn.Linear(embed_dim * 2, embed_dim, linear_bias)
                self.combine_col_embed = nn.Linear(embed_dim * 2, embed_dim, linear_bias)
    ...
    def forward(self, td, phase):
        ...
        demand = torch.cat(
            [
                torch.zeros_like(td["demand"][:, :1], device=td["demand"].device),
                td["demand"],
            ],
            dim=1,
        )
        ...
        return self._embed_with_distance(locs, demand, distance, phase)
    ...
    def _embed_with_distance(self, locs, demand, distance, phase):
        ...
        demand_emb = self.demand_init(demand[..., None])

        combined_row_output = self.combine_row_embed(
            torch.cat([combined_row_emb, demand_emb], dim=-1)
        )
        combined_col_output = self.combine_col_embed(
            torch.cat([combined_col_emb, demand_emb], dim=-1)
        )
