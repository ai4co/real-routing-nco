...
class LazyRMTVRPGenerator(Generator):
    ...
    def generate_demands(self, batch_size: int, num_loc: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def generate_time_windows_with_duration_matrix(self, duration: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def generate_backhaul_class(self, shape: Tuple[int, int], sample: bool = False) -> torch.Tensor:
    ...
    def subsample_problems(self, td: TensorDict) -> TensorDict:
        ...
        if self.use_combinations:
            ...
            if self.variant_preset in list(VARIANT_GENERATION_PRESETS.keys()) and \
               self.variant_preset not in ("all", "cvrp", "single_feat", "single_feat_otw"):
                cvrp_prob = 0
            else:
                cvrp_prob = 0.5
