...
def generate_data_in_chunks(
    ...
    **kwargs
    ...
    for chunk_idx in tqdm(range(num_chunks), desc=f"Generating {problem} data"):
        ...
        log.info(f"Processing chunk {chunk_idx + 1}/{num_chunks} "
...
def generate_env_data_chunk(
    ...
    **kwargs
    ...
    # 메모리 효율적인 샘플링
    sampled_data = sample_data_efficient(
        data_dir, chunk_size, graph_size, **kwargs
    )
...
def generate_mtvrp_data_efficient(
    ...
    **kwargs
    ...
    # 기본 데이터 생성
    demand_linehaul = np.random.randint(1, 10, (dataset_size, num_loc)).astype(np.float32) / 50.0
    
    # 시간 윈도우 생성 (메모리 효율적)
    service_time = (0.15 + 0.03 * np.random.rand(dataset_size, num_loc)).astype(np.float32)
