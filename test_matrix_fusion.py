...
import torch
from rrnco.models.nn.attn_freenet import (
    GatingNeuralAdaptiveBias,
    NaiveNeuralAdaptiveBias,
    AttnFreeNet,
)
...
def test_neural_adaptive_bias_types():
    ...
    attn_net_gating = AttnFreeNet(
        embed_dim=embed_dim, num_layers=2, nab_type="gating", use_duration_matrix=True
    )
    ...
    attn_net_naive = AttnFreeNet(
        embed_dim=embed_dim, num_layers=2, nab_type="naive", use_duration_matrix=True
    )
...
def test_error_handling():
    ...
    try:
        attn_net_invalid = AttnFreeNet(embed_dim=128, nab_type="invalid_type")
